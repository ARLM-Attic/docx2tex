...
$_{2}$, \ldots I$_{M}$\} the set of input variables, I
\begin{figure}[h]
\centering
...
}$,\ldots i$_{jk}$$>$, where i$_{i1}$, i$_{i2}$,\ldots i
$_{in}$, i$_{j1}$, i$_{j2}$, \ldots i$_{jk}$ are the 
values of the corresponding input variables. A pair (t$_{1}$, t$_{
2}$) of test cases is a reliable test pair with respect to statement s
$^{q}$ (where s$^{q}$ represents the q$^{th}$ execution of 
statement s) if t$_{1}$ and t$_{2}$ travels along the same 
execution path until s$^{q}$, the result of s$^{q}$ differs for t
$_{1}$ and t$_{2}$, and t$_{1}$ and t$_{2}$ are 'close' to 
each other (for numeric values the difference should be minimized 
