...
namespace docx2tex.Library
{
    ...
    {
        public static string DocxPath { get; set; }
    }
    ...
    sealed public class Config
    {
        private Config()
        {
            ...
            ConfigXml systemConfig = GetConfig(mainModuleDirPath, "docx2tex.SystemConfig");
            ...
            ConfigXml userConfig = GetConfig(userConfigPath, "docx2tex.UserConfig");
            ...
            ConfigXml documentConfig = GetConfig(docxDirPath, configFileName);
        ...
        private static ConfigXml GetConfig(string moduleDir, string fileName)
        {
            ...
            {
                // config serializer
                XmlSerializer docx2texConfigSerialier = new XmlSerializer(typeof(ConfigXml));

                using (StreamReader systemConfigReader = new StreamReader(configPath))
                {
                    config = (ConfigXml)docx2texConfigSerialier.Deserialize(systemConfigReader);
                }
            }
            return config;
        }
